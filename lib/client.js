...
class Freshdesk {
	...
	createLead(data, cb) {
		makeRequest('POST', this._auth, `${this.baseUrl}/api/v2/leads`, null, data, cb)
	}

	getLead(id, cb) {
		makeRequest('GET', this._auth, `${this.baseUrl}/api/v2/leads/${id}`, null, null, cb)
	}

	listAllLeads(params, cb) {
		makeRequest('GET', this._auth, `${this.baseUrl}/api/v2/leads`, params, null, cb)
	}

	updateLead(id, data, cb) {
		makeRequest('PUT', this._auth, `${this.baseUrl}/api/v2/leads/${id}`, null, data, cb)
	}

	deleteLead(id, cb) {
		makeRequest('DELETE', this._auth, `${this.baseUrl}/api/v2/leads/${id}`, null, null, cb)
	}

	filterLeads(query, cb) {
		const encodedQuery = encodeURI(query)
		makeRequest('GET', this._auth, `${this.baseUrl}/api/v2/search/leads?query="${encodedQuery}"`, null, null, cb)
	}
